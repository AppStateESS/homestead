...
function hms_update(&$content, $currentVersion)
{
    ...
    switch ($currentVersion) {
        ...
        case version_compare($currentVersion, '0.2.31', '<') :
            ...
            PHPWS_Boost::registerMyModule('hms', 'users', $content); // ust need to update the permissions
        ...
        case version_compare($currentVersion, '0.5.7', '<') :
            $db = new PHPWS_DB();
            $result = $db->importFile(PHPWS_SOURCE_DIR . 'mod/hms/boost/updates/00-05-07.sql');
            if (PEAR::isError($result)) {
                return $result;
            }
        case version_compare($currentVersion, '0.5.7', '<'):
            $db = new PHPWS_DB();
            $result = $db->importFile(PHPWS_SOURCE_DIR . 'mod/hms/boost/updates/00-05-08.sql');
            if (PEAR::isError($result)) {
                return $result;
            }
    }
