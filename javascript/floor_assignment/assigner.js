...
var AssignWidget = function(div, semaphore) {
    ...
    this.getOverlay = function() {
        ...
        var output = '<div id="floor_overlay_' + this.bed + '" class="overlay" style="background-color:white;border:1px solid #e3e3e3;padding:1em;position: absolute; top: ' + (offset.top + height) + 'px; left: ' + offset.left + 'px;">';

        output += '<table class="table">';
        output += '<tr><th><label for="username_' + this.bed + '">Username</label></th><td><input id="username_' + this.bed + '" class="form-control username-input" type="text" value="' + this.username + '" /></td></tr>';
        ...
        output += '<tr><td></td><td style="text-align: right"><span id="status_' + this.bed + '" /><button style="margin-right:1em" class="btn btn-success" id="accept_' + this.bed + '"><i class="fa fa-check"></i> Assign</button><button class="btn btn-danger cancel" id="cancel_' + this.bed + '"><i class="fa fa-ban"></i>  Cancel</button></td></tr>';
        output += '</table>';
        ...
        return function() {
            ...
            if (me.overlayShown) {
                ...
                $("#accept_" + me.bed).click(function() {
                    $("#status_" + me.bed).html('<i class="fa fa-2x fa-spinner fa-spin pull-left"></i>');
                    me.syncOverlay();
                    me.submitAssignment()
                });
                $("#cancel_" + me.bed).click(function() {
                    me.toggleOverlayFunc()
                });
            } else {
    ...
    this.submitAssignment = function() {
        ...
        function(data) {
            if (!data.success) {
                $("#message_" + me.bed).html('<div class="alert alert-danger"><i class="fa fa-2x fa-exclamation-circle pull-left"></i> ' + data.message + '</div>');
                $("#status_" + me.bed).html('');
            } else {
                $("#message_" + me.bed).html('<div class="alert alert-success"><i class="fa fa-2x fa-check pull-left"></i> Student Assigned!</div>');
                $("#status_" + me.bed).html('');
                setTimeout(function() {
