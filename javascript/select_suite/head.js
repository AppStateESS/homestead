...
var suite_drop       = 'phpws_form_suite';
...
function handle_hall_change()
{
    ...
    resetDrop(suite_drop);

    disableDrop(floor_drop);
    disableDrop(suite_drop);
...
function handle_floor_change()
{
    ...
    // Set the floor drop down to "loading"
    setSingleOption(suite_drop, "Loading...");
    
    // Assemble the necessary URL
    var requestURL = document.location + '?mod=hms&type=xml&op=get_suites&floor_id=' + floorId;
...
function handle_floor_response()
{
    ...
    setSingleOption(suite_drop, 'Select...');
    ...
    for(var i = 0; i < rooms.childNodes.length; i++){
        ...
        for (j = 0; j < room.childNodes.length; j++){
            ...
            if(sub.nodeName == 'room_list') {
                room_num = sub.firstChild.nodeValue;
                //alert('room_num is ' + room_num);
                var drop = document.getElementById(suite_drop);
                drop.options[drop.options.length] = new Option(room_num, id, false, false);
            }
        }
    }

    enableDrop(suite_drop);
}

function handle_suite_change()
{
    var roomId = document.getElementById(suite_drop).options[document.getElementById(suite_drop).selectedIndex].value;
