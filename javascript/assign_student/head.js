...
var bedroom_drop    = 'phpws_form_bedroom';
...
function handle_hall_change()
{
    ...
    resetDrop(room_drop);
    resetDrop(bedroom_drop);
    resetDrop(bed_drop);

    disableDrop(floor_drop);
    disableDrop(room_drop);
    disableDrop(bedroom_drop);
...
function handle_room_change()
{
    ...
    // Set the floor drop down to "loading"
    setSingleOption(bedroom_drop, "Loading...");
    
    // Assemble the necessary URL
    var requestURL = baseURL + '?mod=hms&type=xml&op=get_bedrooms_with_vacancies&room_id=' + roomId;
...
function handle_room_response()
{
    ...
    setSingleOption(bedroom_drop, 'Select...');

    var response = xmlHttp.responseXML;

    //alert(response);

    var bedrooms = response.firstChild;

    for(var i = 0; i < bedrooms.childNodes.length; i++){
        var bedroom = bedrooms.childNodes[i];
        if(bedroom.nodeType == 3){
            continue;
        }
        for (j = 0; j < bedroom.childNodes.length; j++){
            sub = bedroom.childNodes[j];
            if(sub.nodeType == 3){
                continue;
            }
            if(sub.nodeName == 'id'){
            ...
            if(sub.nodeName == 'bedroom_letter') {
                var bedroom_letter = sub.firstChild.nodeValue;
                //alert('bedroom_letter is ' + bedroom_letter);
                var drop = document.getElementById(bedroom_drop);
                drop.options[drop.options.length] = new Option(bedroom_letter, id, false, false);
            }
        }
    }

    enableDrop(bedroom_drop);
}

function handle_bedroom_change()
{
    // Reset and disable all the lower-order drop downs
    resetDrop(bed_drop);
    disableDrop(bed_drop);
    ...
    // Get the selected value
    var bedroomId = document.getElementById(bedroom_drop).options[document.getElementById(bedroom_drop).selectedIndex].value
    ...
    // Assemble the necessary URL
    var requestURL = baseURL + '?mod=hms&type=xml&op=get_beds_with_vacancies&bedroom_id=' + bedroomId;
...
function handle_bedroom_response()
{
    if(xmlHttp.status != 200){
        alert('An error occurred.. HTTP status code: ' + xmlHttp.status);
    }else{
    ...
    var beds = response.firstChild;

    for(var i = 0; i < beds.childNodes.length; i++){
        var bed = beds.childNodes[i];
        if(bed.nodeType == 3){
            continue;
        }
        for (j = 0; j < bed.childNodes.length; j++){
            sub = bed.childNodes[j];
            if(sub.nodeType == 3){
                continue;
            }
            if(sub.nodeName == 'id'){
            ...
            if(sub.nodeName == 'bed_letter') {
                var bed_letter = sub.firstChild.nodeValue;
                //alert('bed_letter is ' + bed_letter);
                var drop = document.getElementById(bed_drop);
                drop.options[drop.options.length] = new Option(bed_letter, id, false, false);
            }
        }
    }
