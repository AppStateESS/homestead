...
class SubmitRoomChangeRequestCommand extends Command {
    ...
    public function execute(CommandContext $context){
        ...
        if(($cellNum == "" || empty($cellNum)) && !isset($optOut)){
            ...
            $returnMsg = array('error' => 'missing_value', 'value' => 'A phone number to reach you should be provided, if you do not wish to give your phone number please check the box indicating this.');
        ...
        if($reason == '' || empty($reason)){
            ...
            $returnMsg = array('error' => 'missing_value', 'message' => 'A reason must be given for requesting a room change.');
            header("HTTP/1.1 500 Internal Server Error");
            echo json_encode($returnMsg);
            exit;
        }
        ...
        if ($type == 'swap') {
            ...
            foreach ($userToBeds as $node)
            {
              foreach($users as $user)
              {
                if($user == $node['user'])
                {
                  ...
                  $returnMsg = array('error' => 'duplicate_users', 'message' => 'The participant ' . $node['user'] . ' was entered twice.');
                  header("HTTP/1.1 500 Internal Server Error");
                  echo json_encode($returnMsg);
                  exit;
                }
              ...
              foreach ($beds as $bed)
              {
                if($bed == $node['bedId'])
                {
                  ...
                  $returnMsg = array('error' => 'duplicate_bed', 'message' => 'A bed was chosen for more than one user at a time.');
                  header("HTTP/1.1 500 Internal Server Error");
                  echo json_encode($returnMsg);
                  exit;
                }
            ...
            foreach ($userToBeds as $node)
            {
                ...
                try {
                  ...
                  // from being picked.
                  $returnMsg = array('error' => 'invalid_user', 'message' => 'A student with the username '. $switchUsername . ' does not exist in our records.');
                  header("HTTP/1.1 500 Internal Server Error");
                  echo json_encode($returnMsg);
                  exit;
                }
                ...
                if($swapStudent->getGender() != $initGender)
                {
                  ...
                  $returnMsg = array('error' => 'invalid_gender', 'message' => 'Error occurred. Please ensure that all participants are of the same sex.');
                  header("HTTP/1.1 500 Internal Server Error");
                  echo json_encode($returnMsg);
                  exit;
                }
                ...
                if (is_null($swapAssignment)) {
                  ...
                  $returnMsg = array('error' => 'unassigned_user', 'message' => 'The student with username ' . $switchUsername.' does not appear to be assigned at present, so cannot participate in a room change.');
                  header("HTTP/1.1 500 Internal Server Error");
                  echo json_encode($returnMsg);
                  exit;
                }
                ...
                if(!is_null($swapStudentReq)){
                  ...
                  $returnMsg = array('error' => 'already_room_change', 'message' => 'The student with username '. $switchUsername . ' is already involved in a room change and cannot be involved in another at this time.');
                  header("HTTP/1.1 500 Internal Server Error");
                  echo json_encode($returnMsg);
                  exit;
                }
            ...
            foreach($beds as $inputBed)
            {
              ...
              if(!$check)
              {
                ...
                $returnMsg = array('error' => 'invalid_bedId', 'message' => 'Input contained a bed that none of the participants are currently assigned to.');
                header("HTTP/1.1 500 Internal Server Error");
                echo json_encode($returnMsg);
                exit;
              }
