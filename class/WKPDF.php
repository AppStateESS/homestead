...
class WKPDF {
    ...
    public static $PDF_USE_XVFB = false;
    public static $PDF_XVFB_PATH = null;
    ...
    public function setXVFB($path)
    {
        self::$PDF_USE_XVFB = true;
        if (!is_file($path)) {
            throw new \Exception('xvfb executable not found');
        }
        self::$PDF_XVFB_PATH = $path;
    }
    ...
    public function set_html($html)
    {
        ...
        if (!file_put_contents($this->tmp, $html)) {
            throw new \Exception('Could not create file ' . $this->tmp);
        }
    }
